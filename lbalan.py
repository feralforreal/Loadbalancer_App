...
class LoadBalancerApp(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.serverlist.append({'ip': "10.0.0.1", 'mac': "00:00:00:00:00:01", "outport": "1"})  # Appending all given IP's, assumed MAC's and ports of the switch to which servers are connected to the list created
    ...
    def switch_features_handler(self, ev):
        ...
        # 128, OVS will send Packet-In with an invalid buffer_id and
    ...
    def function_for_arp_reply(self, dst_ip, dst_mac):
        ...
        src_ip = self.virtual_lb_ip  # Making the load balancer's IP and MAC as source IP and MAC
        src_mac = self.virtual_lb_mac

        arp_opcode = 2  # ARP opcode is 2 for ARP reply
        hardware_type = 1  # 1 indicates Ethernet i.e., 10Mb
        arp_protocol = 2048  # 2048 means IPv4 packet
        ether_protocol = 2054  # 2054 indicates ARP protocol
        len_of_mac = 6  # Indicates the length of MAC in bytes
        len_of_ip = 4  # Indicates the length of IP in bytes
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:  # If the ethernet frame has eth type as 2054 indicating as ARP packet...
        ...
        print("<========Packet from client: " + str(ip_header.src) + ". Sent to server: " + str(
        ...
        match = parser.OFPMatch(in_port=server_outport_selected, eth_type=eth.ethertype,
                                eth_src=server_mac_selected, eth_dst=self.virtual_lb_mac, ip_proto=ip_header.proto,
        ...
        print("<++++++++Reply sent from server: " + str(server_ip_selected) + ", MAC: " + str(
            server_mac_selected) + ". Via load balancer: " + str(
            self.virtual_lb_ip) + ". To client: " + str(ip_header.src) + "++++++++>")
